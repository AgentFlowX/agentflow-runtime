...
export async function launchHermesDesktop(): Promise<void> {
  if (fakeMode()) {throw new Error('Режим предпросмотра — запуск отключён.')}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  { name: 'system-packages', title: 'Системные пакеты' },
  { name: 'uv', title: 'uv' },
  { name: 'python', title: 'Окружение Python' },
  { name: 'repo', title: 'Репозиторий AgentFlow' },
  { name: 'dependencies', title: 'Зависимости Python' },
  { name: 'node', title: 'Среда Node' },
  { name: 'desktop', title: 'Приложение' }
]

const FAKE_UPDATE_STAGES: FakeStage[] = [
  { name: 'handoff', title: 'Подготовка к обновлению' },
  { name: 'update', title: 'Загрузка последней версии' },
  { name: 'rebuild', title: 'Пересборка приложения' },
  { name: 'install', title: 'Установка обновления' }
]
...
async function runFakeBoot(kind: FakeMode): Promise<void> {
  ...
  try {
    ...
    const cancelled = () => {
      ...
      fakeFail(kind === 'update' ? 'Обновление отменено.' : 'Установка отменена.')
      $route.set('failure')
    ...
    for (const s of stages) {
      ...
      if (s.name === failAt) {
        fakeStage(s.name, 'failed', durationMs, 'Смоделированный сбой для предпросмотра.')
        fakeFail('Смоделированный сбой для предпросмотра (fake boot).')
        $route.set('failure')
