...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Готово' : isUpdate ? 'Обновление AgentFlow' : 'Настройка AgentFlow'

  const description = isUpdate
    ? 'AgentFlow обновляется до последней версии — это займёт пару секунд.'
    : 'Это разовая настройка. Установщик AgentFlow скачивает зависимости и настраивает ваш компьютер. Последующие запуски пропустят этот шаг.'
          ...
          <div className="mb-4">
            <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
              <span className={clsx(bootstrap.status === 'running' && 'shimmer')}>
                {progress.done} из {progress.total} шагов готово
              </span>
        ...
        {showLogs && (
          <div className="flex w-1/2 flex-col border-l border-(--stroke-nous)">
            <div className="flex shrink-0 items-center justify-between border-b border-(--stroke-nous) px-3 py-2 text-xs">
              <span className="font-medium text-foreground/80">Живой вывод</span>
              <span className="tabular-nums text-muted-foreground">{bootstrap.logs.length} строк</span>
            </div>
